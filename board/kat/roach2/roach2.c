...
static int dump_roach2_eeprom(void)
{
        ...
        /*roach user message */
        printf("SN:    ROACH%d.%d batch=%c#%d#%d %s\n",((data_buf[1]) + 1),data_buf[2],data_buf[0],data_buf[3],data_buf[4], (data_buf[2] == CONFIG_ROACH2_REV) ?  "software fixups match" : "INCORRECT uboot loaded");
...
int board_early_init_f(void)
{
  ...
  mtdcr(UIC0PR, 0xffbff7ff);  /* per ref-board manual */
  ...
  mtdcr(UIC1PR, 0xfffffeaf);  /* per ref-board manual */
